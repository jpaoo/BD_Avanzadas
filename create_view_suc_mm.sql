...
union
select pls.id_libro, pl.titulo, 'MM' as Sucursal, pls.cantidad from proyecto_librossucursal@mc pls, proyecto_libros pl where pls.id_libro = pl.id_libro
order by titulo;
...
UNION (
  SELECT ID_ORDENCOMPRA, ID_PROVEEDOR, FECHA, 'MM' AS SUCURSAL FROM PROYECTO_ORDENESCOMPRA@JP
) UNION (
...
UNION (
  SELECT  *  FROM ( SELECT SUM(PC.ID_METODO_PAGO) AS CANTIDAD_PAGOS, MP.NOMBRE AS METODO_PAGO, 'MM' AS SUCURSAL
