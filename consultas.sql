...
SELECT SUM(CANTIDAD_PAGOS) AS CANTIDAD, METODO_PAGO, SUCURSAL FROM (
SELECT SUM(PC.ID_METODO_PAGO) AS CANTIDAD_PAGOS, CL.NOMBRE AS METODO_PAGO, 'JP' AS SUCURSAL
FROM PROYECTO_COMPRAS PC, PROYECTO_METODOS_PAGO CL
WHERE PC.ID_METODO_PAGO = CL.ID_METODO_PAGO
GROUP BY CL.NOMBRE
UNION (
  SELECT SUM(PCM.ID_METODO_PAGO) AS CANTIDAD_PAGOS, CLM.NOMBRE AS METODO_PAGO, 'MC' AS SUCURSAL
  FROM PROYECTO_COMPRAS@MC PCM, PROYECTO_METODOS_PAGO@MC CLM
  WHERE PCM.ID_METODO_PAGO = CLM.ID_METODO_PAGO
  GROUP BY CLM.NOMBRE
)
UNION (
  SELECT SUM(PCK.ID_METODO_PAGO) AS CANTIDAD_PAGOS, CLK.NOMBRE AS METODO_PAGO, 'KR' AS SUCURSAL
  FROM PROYECTO_COMPRAS@KR PCK, PROYECTO_METODOS_PAGO@KR CLK
  WHERE PCK.ID_METODO_PAGO = CLK.ID_METODO_PAGO
  GROUP BY CLK.NOMBRE
)
UNION (
  SELECT SUM(PMM.ID_METODO_PAGO) AS CANTIDAD_PAGOS, PCMM.NOMBRE AS METODO_PAGO, 'MM' AS SUCURSAL
  FROM PROYECTO_COMPRAS@MM PMM, PROYECTO_METODOS_PAGO@MM PCMM
  WHERE PMM.ID_METODO_PAGO = PCMM.ID_METODO_PAGO
  GROUP BY PCMM.NOMBRE
)) GROUP BY METODO_PAGO, SUCURSAL;
