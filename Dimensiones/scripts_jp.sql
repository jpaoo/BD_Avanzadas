...
select DS.NOMBRE AS SUCURSAL, SUM(HV.SUM_MONTO)  AS MONTO_VENDIDO_MES from D_SUCURSAL DS, H_VENTAS HV where HV.ID_SUCURSAL = DS.ID_SUCURSAL AND HV.ID_TIEMPO IN (SELECT id_tiempo from d_tiempo where mes=2 and anio = 2018) GROUP BY DS.NOMBRE
